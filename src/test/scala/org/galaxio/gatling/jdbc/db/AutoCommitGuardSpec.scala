...
class AutoCommitGuardSpec extends AnyFlatSpec with Matchers with BeforeAndAfterAll {
  ...
  // Regression: the guard must hold for every construction path, not only the companion `apply` — a public class with an
  // unchecked primary constructor would let `new JDBCClient(...)` bypass it entirely, reproducing the OK-without-persistence
  // defect this whole story exists to close.
  "new JDBCClient" should "also reject a pool configured with autoCommit=false" in {
    val cfg          = H2.config("autocommit_guard_reject_ctor", 2)
    cfg.setAutoCommit(false)
    val rejectedPool = new HikariDataSource(cfg)
    val pool         = Executors.newFixedThreadPool(1)
    try {
      val ex = the[IllegalArgumentException] thrownBy new JDBCClient(rejectedPool, pool)
      ex.getMessage should include("auto-commit")
    } finally {
      rejectedPool.close()
      pool.shutdown()
