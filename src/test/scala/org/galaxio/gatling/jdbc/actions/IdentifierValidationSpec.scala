...
import org.galaxio.gatling.jdbc.actions.actions.{BatchUpdateAction, Columns, DBInsertActionBuilder, BatchActionBuilder}
...
class IdentifierValidationSpec extends AnyFlatSpec with Matchers with JdbcActionSpecFixture {
  ...
  "a batch with a malicious table name" should "fail the request with the table intact" in {
    seed()
    val stats = new RecordingStatsEngine
    val tc    = buildRealTestContext(dbName, 2, config, stats)
    try {
